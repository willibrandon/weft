...
public sealed class WeftTools(WeftBridge bridge)
{
    ...
    public async Task<string> ListBlocksAsync(
        [Description("Session name, session id, or session:tab target. Omit for every session.")] string? target,
    ...
    public async Task<string> CreateBlockAsync(
        [Description("Block, tab, or session to split from. Omit for the active block of the latest session.")] string? target,
        [Description("Command and arguments to run, one string per argument. Omit to start the default shell.")] string[]? command,
        [Description("Split below instead of to the right.")] bool below,
        [Description("Working directory for the command.")] string? cwd,
        CancellationToken cancellationToken)
    {
        ControlClient client = await bridge.ClientAsync(cancellationToken).ConfigureAwait(false);
    ...
    public async Task<string> RunAsync(
        ...
        [Description("Session to run in. Omit for the latest session, which is created when none exists.")] string? target,
        [Description("Working directory.")] string? cwd,
        [Description("Milliseconds to wait for exit before returning. Defaults to 120000.")] int? timeoutMs,
        CancellationToken cancellationToken)
    {
        ControlClient client = await bridge.ClientAsync(cancellationToken).ConfigureAwait(false);
    ...
    public async Task<string> CaptureAsync(
        [Description("Block id or session:tab.block target.")] string target,
        [Description("Scrollback lines to include above the screen. Defaults to 0.")] int? history,
        CancellationToken cancellationToken)
    {
        ControlClient client = await bridge.ClientAsync(cancellationToken).ConfigureAwait(false);
    ...
    public async Task<string> WaitForAsync(
        [Description("Block id or session:tab.block target.")] string target,
        [Description("Regular expression matched against each screen line.")] string? pattern,
        [Description("Return when the process exits.")] bool exit,
        [Description("Return once the output revision exceeds this value, as reported by capture.")] long? revision,
        [Description("Milliseconds to wait. Defaults to 30000.")] int? timeoutMs,
        CancellationToken cancellationToken)
    {
        ControlClient client = await bridge.ClientAsync(cancellationToken).ConfigureAwait(false);
